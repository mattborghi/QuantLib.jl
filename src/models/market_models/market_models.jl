...
function get_covariance(mm::AbstractMarketModel, i::Int)
  if isempty(mm.covariance)
    ...
    @simd for j = 1:mm.numberOfSteps
      @inbounds mm.covariance[j] = mm.pseudoRoots[j] * mm.pseudoRoots[j]'
    end
...
function total_covariance(mm::AbstractMarketModel, endIndex::Int)
  if isempty(mm.totalCovariance)
    ...
    @simd for j = 2:mm.numberOfSteps
      @inbounds mm.totalCovariance[j] = mm.totalCovariance[j-1] + mm.covariance[j]
    end
